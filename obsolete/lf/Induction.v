...
Proof.
 ...
 - intros n. simpl. rewrite IHm'. reflexivity.
Qed.
...
Theorem mul_add: forall m n: nat,
 n * S m = n + n * m.
Proof.
 intros.
...
Theorem mul_add2: forall m n: nat,
 S n * m = n * m + m.
Proof.
 intros.
 induction n as [|m' IHm'].
...
Proof.
 ...
 - simpl. rewrite IHn'. rewrite mul_add. reflexivity.
Qed.
(** [] *)
...
Proof.
 intros.
 induction n as [|n' IHn'].
 - rewrite add_comm. rewrite add_0_r. 
   assert (H: 0 * p = 0).
   { rewrite mul_comm. rewrite mul_0_r. reflexivity. }
   rewrite H.
   rewrite add_comm.
   rewrite add_0_r.
   reflexivity.
...
Lemma nat_incr: forall n, bin_to_nat (nat_to_bin (S n)) = S n.
Proof.
 intros.
 induction n as [|n' IHn'].
 - reflexivity.
...
Lemma incr_bin_lemma: forall n, nat_to_bin (S n) = incr (nat_to_bin n).
Proof.
 reflexivity.
Qed.

Lemma double_lemma: forall n, nat_to_bin (2 * n + 1) = incr (double_bin (nat_to_bin n)).
Proof.
 intros.
 induction n as [|n' IHn'].
 - reflexivity.
...
Lemma double_lemma2: forall n, nat_to_bin (2 * n) = double_bin (nat_to_bin n).
Proof.
 induction n as [|n' IHn'].
 - reflexivity.
 - simpl. rewrite add_0_r.
   replace (n' + S n') with (2 * n' + 1).
   rewrite double_lemma.
   rewrite double_incr_bin.
   reflexivity.
   rewrite <- plus_n_Sm.
   rewrite <- plus_n_Sm.
...
Lemma normalize_lemma: forall b, double_bin (normalize b) = normalize (B0 b).
Proof.
 intros.
 induction b as [|b' IHb'|c' IHc'].
 - reflexivity.
 - unfold double_bin.
   unfold normalize. reflexivity.
 - unfold double_bin.
   unfold normalize. reflexivity.
 - unfold double_bin.
   unfold normalize. reflexivity.
...
Lemma normalize_lemma_2: forall c', normalize (B1 c') = B1 (normalize c').
Proof.
 intros.
 induction c'.
 - reflexivity.
...
Proof.
 ...
 induction b as [|b' IHb'|c' IHc'].
 - reflexivity.
 - replace (bin_to_nat (B0 b')) with (bin_to_nat b' + bin_to_nat b').
   replace (bin_to_nat b' + bin_to_nat b') with (2 * (bin_to_nat b')).
   rewrite double_lemma2.
   rewrite IHb'.
   rewrite normalize_lemma.
   reflexivity.
   simpl.
   rewrite add_0_r.
   reflexivity.
 ...
 - simpl.
   ...
   rewrite -> plus_n_Sm.
   rewrite -> plus_n_Sm.
   replace (bin_to_nat c' + (bin_to_nat c' + 1)) with (2 * (bin_to_nat c') + 1).
   rewrite double_lemma.
   rewrite <- IHc'.
   rewrite <- double_lemma2.
   replace (B1 (nat_to_bin (bin_to_nat c'))) with (nat_to_bin (2 * bin_to_nat (c') + 1)).
   rewrite <- plus_n_Sm.
   rewrite  incr_bin_lemma.
   rewrite add_0_r.
   rewrite double_lemma2.
   ...
   replace (B1 (nat_to_bin (bin_to_nat c'))) with 
   rewrite normalize_lemma.
   unfold normalize.
   replace (incr (normalize (B0 c'))) with (B1 (normalize c')).
   reflexivity.
   induction c'.
   + reflexivity.
   + simpl.
