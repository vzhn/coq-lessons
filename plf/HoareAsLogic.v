...
(* 
  derivable P <{ skip }> (assert_of_Prop True)
  (assert_of_Prop True) ->> P
*)
...
Proof.
 intros c.
 induction c; intros.
 - apply H_Consequence_post with (Q' := P). constructor.  constructor.
 - apply H_Consequence_pre with (P' := (fun st => True) [x |-> a]). 
   constructor. constructor.
 - apply H_Consequence_pre with (P' := fun st => True).
   eapply H_Seq.
   + apply IHc2.
   + apply IHc1.
   + constructor.
 - apply H_Consequence_pre with (P' := fun st => True).
   ...
   + constructor.
 - apply H_Consequence_pre with (P' := fun st => True).
   apply H_Consequence_post with (Q' := fun st => True  /\ ~((fun st' => (beval st b) = true) st)).
   eapply H_While.
   + apply IHc.
   + constructor.
   + constructor.
...
Proof.
 induction c; intros.
 - apply H_Consequence_post with (Q' := fun st => False). constructor. intros. destruct H.
 - apply H_Consequence_pre with (P' := Q [x |-> a]).
 ...
 - eapply H_If.
   ...
   + simpl.
     ...
     * apply IHc2.
     * intros. destruct H. assumption.
 - apply H_Consequence_post with (Q' := fun st => False  /\ ~((fun st' => (beval st b) = true) st)).
   + simpl.
     apply H_While.
     apply H_Consequence_pre with (P' := fun st : state => False).
     * apply IHc. 
...
Proof.
  ...
  - Print derivable.
     ...
     * intros. apply H.
     * intros. specialize HT with (st := st) (st' := st).   
       apply HT. 
       ** constructor.
       ** assumption.
  - eapply H_Consequence. constructor.
    + intros.
      specialize HT with (st := st) (st' := (x !-> aeval st a ; st)).   
      apply HT.
      constructor.
      * reflexivity.
      * assumption.
