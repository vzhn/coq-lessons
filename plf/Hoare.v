...
(* 
hoare_asgn
     : forall (Q : Assertion) (X : string) (a : aexp), {{Q [X |-> a]}} X := a {{Q}}
*)
...
Proof.
 ...
 intros.
 simpl in H.
 specialize H with (st:= (X !-> 0; empty_st)).
 specialize H with (st' := (X !-> 1; X !-> 0;  empty_st)).
 assert (HH: (X !-> 0) =[ X := X + 1 ]=> (X !-> 1; X !-> 0)).
 { constructor. simpl. reflexivity. }
 apply H in HH.
 + simpl in HH.
   rewrite t_update_eq in HH. discriminate HH.
 + constructor.  
Qed.
(* FILL IN HERE
...
Search total_map.
(* 
 t_update_same: forall (A : Type) (m : total_map A) (x : string), (x !-> m x; m) = m
 t_update_eq: forall (A : Type) (m : total_map A) (x : string) (v : A), (x !-> v; m) x = v
 t_update_shadow:
...
Proof.
 ...
 intros.
 inversion H. subst.
 destruct H0 as [HA HB]. subst. 
 apply conj.
 - rewrite <- t_update_same with (x := X) in HA.
   rewrite <- t_update_shadow with (v1 := aeval st a) in HA.
   assumption.
 - rewrite t_update_eq.
...
Proof.
 ...
 inversion H. subst.
 apply conj.
 - rewrite t_update_shadow, t_update_same. assumption.
 - rewrite t_update_shadow, t_update_same, t_update_eq. reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  specialize H with 
    (n := 1) (a := <{ X + 1 }>)
    (st := empty_st) (st' := (Y !-> 4 ; X !-> 3 ; empty_st)).
  assert (H1:  empty_st =[ X := 3; Y := X + 1 ]=> (Y !-> 4; X !-> 3)).
  { 
    apply E_Seq with (st' :=  (X !-> 3 ; empty_st)). 
...
Proof.
 ...
 + simpl.
   ...
   - unfold "->>". 
     ...
     simpl. assumption.
 + simpl.   
   unfold valid_hoare_triple, t_update.
   intros.
   - inversion H; subst.
     * destruct H0 as [HA HB].
       rewrite negb_true_iff in HB.
       apply not_false_iff_true in HB.
       apply eqb_eq in HB. 
       rewrite HB in *.
       rewrite add_0_r in HA. 
...
Proof.
 ...
 - intros. inversion H. subst.
   ...
   + reflexivity.
   + constructor.
 - intros. specialize H with empty_st (RError).
   assert (HH: empty_st =[ assert false ]=> RError).
   { constructor. simpl. reflexivity. }
   apply H in HH.
   + inversion HH.
     destruct H0.
...
Proof.
 ...
 inversion H0. subst.
 assert (HH: st =[ assert b ]=> RNormal st).
...
(* 
hoare_seq
     : forall (P Q R : Assertion) (c1 c2 : com), {{Q}} c2 {{R}} -> {{P}} c1 {{Q}} -> {{P}} c1; c2 {{R}}
*)
 ...
 + inversion H6; subst.
   - inversion H8; subst.
     * exists st'0.
       ...
       ** reflexivity.
       ** simpl. constructor. 
     * inversion H4. inversion H3. inversion H8. subst.
       simpl in *.
       apply eqb_eq in H14.
