...
Example prop1: ~bool_step_prop1.
Proof.
 unfold bool_step_prop1, not.
 intros.
 inversion H.
Qed.
...
Example prop2: ~bool_step_prop2.
Proof.
 unfold bool_step_prop2, not. intros.
 inversion H.
Qed.
...
Proof.
 ...
 - left. constructor.
 - right.
   destruct IHt1 as [HA1 | HB1].
   + inversion HA1; subst.
     * destruct IHt2 as [HA2 | HB2].
...
(** **** Exercise: 3 stars, standard, optional (properties_of_altered_step)
    ...
    - Is the [step] relation still deterministic?  Write yes or no and
      ...
      I think that it is still deterministic
...
(* FILL IN HERE
   - In general, is there any way we could cause strong progress to
     ...
     if we remove some if-related constructor we could come into situation
     when we have not-value and unable to progress forward
...
Proof.
  ...
  + unfold step_normal_form, normal_form, not in *.
    ...
    * inversion P11; subst.
      ...
      ** assert (HH: y0 = y).
         { apply (step_deterministic x y0 y H0 H). }
         subst. assumption.
    * apply P22.
Qed.
(** [] *)
...
(* 
Inductive multi (X : Type) (R : relation X) : relation X :=
    multi_refl : forall x : X, multi R x x | multi_step : 
   forall x y z : X, R x y -> multi R y z -> multi R x z.
*)
...
Proof.
 ...
 - inversion H; subst.
   + inversion H0; subst.
     * assert (HH: P (C n1) (C n2) --> C (n1 + n2)). 
       { constructor. }
       apply multi_R in HH.
       assumption.
     * assert (HH: P (C n1) (P t1 t0) -->* P (C n1) (C (n0 + n3))).
       { apply multistep_congr_2.  
         + constructor. 
         + assumption.
       }
       apply multi_trans with (P (C n1) (C (n0 + n3))).
       ** assumption.  
       ** apply multi_R.
          apply ST_PlusConstConst.
   + assert (HH1: P (P t0 t3) t2 -->* P (C (n0 + n3)) t2).
     { apply multistep_congr_1. assumption. }
     apply multi_trans with (P (C (n0 + n3)) t2).
     * assumption.
