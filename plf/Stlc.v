...
Inductive substi (s : tm) (x : string) : tm -> tm -> Prop :=
  | s_var1: substi s x (tm_var x) s
  | s_var2 (y: string) (H: y <> x): substi s x (tm_var y) (tm_var y)
  | s_app (l ls r rs: tm)
          (HL: substi s x l ls) 
          (HR: substi s x r rs)
          : substi s x <{ l r }> <{ ls rs }>

  | s_abs1 (T: ty) (body: tm) 
           : substi s x <{\x:T, body}> <{\x:T, body}>

  | s_abs2 (y: string) (T: ty) (b1: tm) (b2: tm)
           (H: x <> y)
           (HB: substi s x b1 b2)
           : substi s x <{\y:T, b1}>  <{\y:T, b2}> 
 ...
 - generalize dependent t'.
   ...
   + destruct (x0 =? s0)%string eqn:E.
     ++  apply eqb_eq in E. subst.
         unfold subst.
        destruct (s0 =? s0)%string eqn:E.
        +++ constructor.
        +++ apply eqb_neq in E. unfold not in E. exfalso. apply E. reflexivity.
     ++
       specialize IHt with <{ [x0 := s] t0 }>.
       assert (HH: <{ [x0 := s] t0 }> = <{ [x0 := s] t0 }>).
       { reflexivity. } 
       apply IHt in HH.
       subst.
       simpl.
       rewrite E.
       apply s_abs2 with (y := s0) (x := x0) (s := s) (T := t) (b1 := t0) (b2 :=  <{ [x0 := s] t0 }>).
  ...
  - generalize dependent t'.
    ...
    + simpl.
      ...
      ++ inversion H; subst.
         +++ reflexivity.
         +++ apply eqb_eq in E. subst.
             unfold not in H1. destruct H1. reflexivity.
      ++ inversion H; subst.
         +++ apply eqb_neq in E. destruct E. reflexivity.
         +++ reflexivity. 
    + inversion H; subst.
      apply IHt1 in HL.
      apply IHt2 in HR.
    ...
    + destruct (x0 =? s0)%string eqn:E.
      ++ simpl. rewrite E. 
         inversion H; subst.
         +++ reflexivity.
         +++ apply eqb_eq in E. destruct H4. assumption.
      ++ simpl. rewrite E.
         inversion H; subst.
         +++ apply eqb_neq in E. destruct E. reflexivity.
         +++ apply IHt in HB. subst. reflexivity.
    + inversion H; subst.
      simpl. reflexivity.
    + inversion H; subst.
      simpl. reflexivity.
