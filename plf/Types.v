...
Proof with eauto.
 ...
 +  
   ++ inversion H0; subst.
     ...
     +++ inversion H2; subst.
         ...
         ++++ inversion H.
         ++++ inversion H; subst.
              destruct (value_is_nf <{ succ (succ t1) }> ).
 ...
 + inversion H; subst.
   ++ inversion H0.
   ++ Check value_is_nf. destruct (value_is_nf <{ succ (y1) }> ).
 ...
 + inversion H; subst.
  ...
  ++ specialize IHstep with (y1 := t1'0).
     apply IHstep in H2. rewrite H2.
     reflexivity.
Qed.
...
Proof.
  ...
  - (* succ t1 *)
   Check nat_canonical.
   destruct IHHT as [HV | HE].
   + apply (nat_canonical t1) in HT.
     * left. auto.
     * auto. 
   + destruct HE as [t' HE].
     right.
  ...
  - (* pred t1 *)
     ...
     * right.
       ...
       auto.
   - destruct IHHT as [HV | HNEXT].
     + right. apply (nat_canonical t1) in HT.
       ++ inversion HT.
...
Proof.
  ...
  induction HT;
    ...
    - (* T_If *) inversion HE; subst; clear HE.
     ...
     - (* succ *) inversion HE; subst.
      + apply IHHT in H0.
        apply T_Succ. assumption.
     - (* pred *) inversion HE; subst.
      + assumption.
...
Proof.
 ...
 unfold not. intros.
 specialize H with 
   (t := <{ if true then true else 0 }>)
   (t' := <{ true }>)
   (T := Bool).
 assert (HH: 
   <{ if true then true else 0 }> --> <{ true }> /\ |-- <{ true }> \in Bool
  ).
 { 
   split.
   + constructor.
