...
Proof.
 ...
 - intros.
   inversion H.
   subst.
   assert (HH: st'0 =[ skip ]=> st'0).
   { constructor. }
   inversion H5.
   subst. apply H2.
 - intros.
   apply E_Seq with (st := st) (st' := st').
   + apply H. 
   + apply E_Skip.
Qed.
(** [] *)
...
Proof.
 ...
 - intros.
   ...
   + apply E_IfFalse.
     ...
     * assumption.
   + apply E_IfTrue.
     * simpl in H5. rewrite <- negb_true_iff in H5. rewrite negb_involutive in H5. assumption.
...
Proof.
 ...
 - intros.
   ...
   exfalso. apply H0.
 - intros.
   assert (HH:  bequiv <{ true }> <{ true }>).
   { unfold bequiv. simpl. intros. reflexivity. }
   apply while_true_nonterm with (c:= <{ skip }>) (st := st) (st' := st') in HH.
   unfold not in HH.
   apply HH in H0.
   contradiction.
Qed.
...
Proof.
 ...
 - intros.
   inversion H. subst.
   inversion H2. subst.
   apply E_Seq with (st:=st) (st':=st'1).
   * assumption.
   * apply E_Seq with (st:=st'1) (st':=st'0).
     ** assumption.
     ** assumption.
 - intros.
   inversion H. subst.
...
Proof.
 ...
 - intros.   
   ...
   rewrite H in H4.
   rewrite H0 in H7.
   apply E_Seq with (st := st) (st' := st'0).
   + assumption. 
   + assumption.
 - intros.
   inversion H1. subst.
...
Proof.
 ...
 split.
 - intros.
   unfold cequiv in H0, H1.
   unfold bequiv in H.
   inversion H2; subst.
   + apply E_IfTrue.
     * rewrite <- H. assumption.
     * rewrite H0 in H9. assumption.
   + apply E_IfFalse.
     * rewrite <- H. assumption.
     * rewrite H1 in H9. assumption.
 - intros.
   unfold cequiv in H0, H1.
   unfold bequiv in H.
   inversion H2; subst.
...
Proof.
  ...
  - (* while *)
    ...
    + split.
      * intros.
        unfold bequiv in H. specialize H with st.
        inversion H0; subst.
        ** constructor.
        ** simpl in H. rewrite H in H3. discriminate H3.
      * intros. 
        unfold bequiv in H. specialize H with st.
        inversion H0; subst.
        apply E_WhileFalse.
...
(* 
  ...
  refl_bequiv
*)
Check refl_cequiv: forall c : com, cequiv c c.
Check refl_bequiv: forall b : bexp, bequiv b b.
Check CIf_congruence.
Check atrans_sound optimize_0plus_aexp.

(* forall (a : aexp) (st : state), aeval st a = aeval st (optimize_0plus_aexp a) *)
...
Proof.
 ...
 unfold aequiv.
 intros.
 induction a; try ( simpl; try (rewrite IHa1, IHa2); reflexivity).
 - simpl.
   rewrite IHa1, IHa2. 
...
Proof.
 ...
 induction b; try (
   simpl; 
   try (rewrite (optimize_0plus_aexp_sound a1 st); rewrite (optimize_0plus_aexp_sound a2 st)); reflexivity 
 ).
 + simpl. f_equal. apply IHb. 
 + simpl. rewrite IHb1, IHb2. reflexivity.
Qed.

Check CAsgn_congruence.
Check CSeq_congruence.
(* CSeq_congruence
     : forall c1 c1' c2 c2' : com,
...
*)
Check CWhile_congruence.
(* 
CWhile_congruence
     : forall (b b' : bexp) (c c' : com),
       bequiv b b' ->
       cequiv c c' -> cequiv <{ while b do c end }> <{ while b' do c' end }>
*)
...
Proof.
 ...
 intros.
 induction c.
 + simpl. unfold cequiv. intros. split.
   * intros. assumption.
   * intros. assumption.
 + apply CAsgn_congruence. 
   unfold cequiv, aequiv.
   intros. apply (optimize_0plus_aexp_sound a st).
...
Check fold_constants_aexp_sound.
Check atrans_sound.
(** Prove that this optimizer is sound. *)
Print fold_constants_aexp.
...
(* 
t_update_neq:
  forall (A : Type) (m : total_map A) (x1 x2 : string) (v : A), x1 <> x2 -> (x1 !-> v; m) x2 = m x2
*)
...
Proof.
 intros.
 induction H; simpl; try (rewrite IHvar_not_used_in_aexp1, IHvar_not_used_in_aexp2; reflexivity).
 - reflexivity.
...
Proof.
 unfold not, cequiv.
 intros.
 specialize H with (st := empty_st) (st' := empty_st).
 destruct H.
 Check while_true_nonterm.
 assert (HH: empty_st =[ skip ]=> empty_st).
 { constructor. }
 apply H0 in HH.
 apply while_true_nonterm in HH. 
 * assumption.
 * apply refl_bequiv.
Qed.
(** [] *)
...
Search t_update_permute.

Search total_map.

(* 
aeval_weakening:
  forall (x : string) (st : total_map nat) (a : aexp) (ni : nat),
  var_not_used_in_aexp x a -> aeval (x !-> ni; st) a = aeval st a
*)
...
Proof.
 ...
 - intros.
   ...
   apply E_Seq with (st := st) (st' := (Y !-> n0; st)).
   * constructor.
   * rewrite t_update_permute with (x1 := Y) (x2 := X).
     + constructor.
     + discriminate.
 - intros.
   inversion H. subst.
   ...
   apply E_Seq with (st := st) (st' := (X !-> n0; st)).
   * constructor.
   * rewrite t_update_permute with (x1 := X) (x2 := Y).
     + constructor.
     + discriminate.
Qed.
(** [] *)
...
Proof. 
 ...
 right.
 intros.
 specialize H with (st := empty_st).
 specialize H with (st' := (Y !-> 3; X !-> 2)).
 destruct H as [HL HR].
 apply HR in HL.
Admitted.
(** [] *)
