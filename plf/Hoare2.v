...
Proof.
  ...
  (* 
    {{assert_of_Prop True}} while X <> 0 do X := X - 1 end
    {{fun st : state => Aexp_of_aexp X st = Aexp_of_nat 0 st}}
  *)
  ...
  (* 
    ...
    ?Q' ->> (fun st : state => Aexp_of_aexp X st = Aexp_of_nat 0 st)
  *)
  - apply hoare_while.
...
Proof.
 ...
 - unfold not. intros.
   destruct (st X).
   + apply nle_succ_0 in H1. assumption. 
   + destruct n.
     * apply le_S_n in H1.
       apply le_0_r in H1. discriminate H1.
     * discriminate H0.
 - apply parity_ge_2 in H0. rewrite <- H. assumption.
 - apply parity_lt_2 in H0. rewrite <- H. symmetry. assumption.
Qed.
  ...
  <{
    ...
    {{ a = a }}
      ...
      while X <> 0 && Y <> 0 do
             ...
             {{ X <> 0 /\ Y <> 0 /\ a = (Z + 1) + (X - 1) /\
                b = (Z + 1) + (Y - 1) /\ 
                (Z + 1) + (ap2 min (X - 1) (Y - 1)) = (ap2 min ((Z + 1) + (X - 1)) ((Z + 1) + (Y - 1))) }}
        X := X - 1
             {{ a = (Z + 1) + X /\
                b = (Z + 1) + (Y - 1) /\ 
               (Z + 1) + (ap2 min X (Y - 1)) = (ap2 min ((Z + 1) + X) ((Z + 1) + (Y - 1))) 
             }};
        Y := Y - 1
             {{ 
                a = (Z + 1) + X /\
                b = (Z + 1) + Y /\ 
                (Z + 1) + (ap2 min X Y) = (ap2 min ((Z + 1) + X) ((Z + 1) + Y)) 
             }};
        Z := Z + 1
             {{ 
                a = Z + X /\
...
Proof.
 verify.
 + rewrite <- add_comm with  (m:=st X) (n := 1).
   rewrite <- fib_eqn.
   - unfold pred.
     * destruct (st X).
       ** reflexivity.
 ...
 + induction (st X).
   ...
   * rewrite fib_eqn.
     ...
     ** unfold ">"%nat. apply lt_0_succ.
 + unfold not in H.
   destruct (eq_dec (st X) (S n)).
   - rewrite e.
     simpl. 
     rewrite sub_0_r. reflexivity.
   - unfold not in n0.
...
Theorem is_wp_example :
  is_wp (Y <= 4) <{X := Y + 1}> (X <= 5).
Proof.  
  unfold is_wp, valid_hoare_triple.
  split.
  - intros.
    inversion H. subst.
    simpl in *.
...
Check hoare_consequence_post.
(* 
hoare_consequence_post
     : forall (P Q Q' : Assertion) (c : com), {{P}} c {{Q'}} -> Q' ->> Q -> {{P}} c {{Q}}
*)

Check hoare_consequence_pre.
(* 
hoare_consequence_pre
     : forall (P P' Q : Assertion) (c : com), {{P'}} c {{Q}} -> P ->> P' -> {{P}} c {{Q}}
*)
