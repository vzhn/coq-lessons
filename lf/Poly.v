...
match l with
| nil => ([], [])
| lh::lt => match split lt with
                 | (a, b) => ((fst lh)::a, (snd lh)::b)
                 end
end.
...
match l with
| nil => (nil, nil)
| hd :: tl => let rest := partition test tl in
            if test hd
            then (hd :: fst rest, snd rest)
            else (fst rest, hd :: snd rest)
end           
.
...
Lemma fold_incr: forall X (l: list X) (n: X),
...
Definition mult (n m : cnat) : cnat :=
  fun (X : Type) (succ : X -> X) (zero : X) => ((n X) ((m X) succ)) zero.
